...
```{r}
summary_forms <-
  sapply(c("~I(HighEdHH==1)", "~EqIncDOp", "~I(Ten1<=2)", "~I(Gorx == 7)"),
         as.formula)
...
H_fct <-
  (H_full[1, c(1, 3, 4)] %>% simplify2array %>% as.data.frame)[5:8, ]
H_fct <- (H_fct * 100)  %>% round(digits = 2)
colnames(H_fct) <-
  c("% academic degree", "% owning home", "% in London")
...
FT <- formattable(
  ...
  list(
    ...
    "SE  " = smallfont,
    "% in London" = my_color_bar("lightskyblue", 100, function(x) {
      x / 100
    }),
    "SE   " = smallfont
  )
