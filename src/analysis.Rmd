...
The following code may be uncommented to obtain an analysis that consider lower class academic households separately
```{r}
# assign_reckwitz_class <- function(x, upper_threshold = 2.5) {
#   # Since accessing columns by name in mutate is not so easy
#   # Assuming x[1] is EqIncDOp and x[2] is HighEdHH
...
#     x %>% rowwise %>% mutate(ReckwitzClass = factor(
#       assign_reckwitz_class(c(EqIncDOp, HighEdHH), upper_threshold = upper_threshold),
#       levels = c("lower", "academic lower", "old middle", "new middle", "upper")
#     )))
...
# population weighted
svymi_per_capita <- svydesign(ids = ~1,  weights = lcf$weighta * lcf$A049, data = imputationList(mil[2:7]))

```

# Recalibrate weights for personal footprints

```{r}
lcf <- mil$`1`
svy <- svydesign(ids = ~1,  weights = lcf$weighta, data = lcf)
lcf$GHG <- lcf %>% select(starts_with("GHG")) %>% rowSums
svy <- svydesign(ids = ~1,  weights = lcf$weighta, data = lcf)
svy_per_capita <- svydesign(ids = ~1,  weights = lcf$weighta * lcf$A049, data = lcf)
```


```{r}
ghg_per_person_direct = svytotal(~GHG, svy)[[1]] / svytotal(~A049, svy)[[1]]
ghg_per_person_weighted = svymean(~I(GHG/A049), svy)[[1]]

```


```{r}
svyby(~GHG, ~ReckwitzClass, svy, svytotal)$GHG * 52 / 1e3 / svyby(~A049, ~ReckwitzClass, svy, svytotal)$A049



svyby(~I(GHG/A049), ~ReckwitzClass, svy, svymean)$GHG * 52 / 1e6
...
CO2_full <-
  sapply(as.character(c(1:12)), function(x)
    MIcombine(with(
      svymi_per_capita, svyby(coicop_to_ghg_formula(x), ~ ReckwitzClass, svymean)
    )))
...
altbind <- function(df1, df2) {
  ...
  for (i in 2:ncol(df1)) {
    ...
    cns <-
      c(cns, colnames(df1)[i], paste("SE", paste(rep(" ", i - 1), collapse = ""), sep =
                                       ""))
  }
  colnames(df3) <- cns
...
```{r}
summary_forms <-
  sapply(c("~I(HighEdHH==1)", "~EqIncDOp", "~I(Ten1<=2)", "~A049"),
         as.formula)
...
H_age <- H_age %>% round(digits = 2)
colnames(H_age) <- c("Average Household Size")
...
H_age_SE <-
  sapply(H_full[2, c(4)], function(x)
    sqrt(diag(x))) %>% as.data.frame
H_age_SE <- H_age_SE %>% round(digits = 2)
...
FT <- formattable(
  ...
  list(
    ...
    "SE  " = smallfont,
    "Average Household Size" = my_color_bar("lightskyblue", 100, proportion),
    "SE   " = smallfont
  )
)
...
# unclear what weights should be applied here !!! ???
...
internet_full <-
  MIcombine(with(svymi_per_capita, svyby(
